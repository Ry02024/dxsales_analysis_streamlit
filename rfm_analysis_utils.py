...
def calculate_rfm(sales_data, item_data, category_data):
    ...
    rfm_item = rfm.merge(item_data[['商品ID', '商品カテゴリID']], on='商品ID', how='left').rename(columns={'商品カテゴリID_x': '商品カテゴリID'})
    rfm_full = rfm_item.merge(category_data[['商品カテゴリID', '商品カテゴリ名']], on='商品カテゴリID').rename(columns={'商品カテゴリ名_x': '商品カテゴリ名'})
    return rfm_full
